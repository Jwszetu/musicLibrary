...
function DisplaySongs({ songs, onPlayNow, onAddToQueue }) {
  ...
  const handlePlayNow = (song) => {
    ...
    const playable = song.links.find(
      (l) => isYouTubeUrl(l.url) || isSpotifyUrl(l.url)
    );
    if (!playable) return;
    const meta = {
      title: song.title,
                  ...
                  {song.links.map((link) => {
                    ...
                    return (
                      <a
                        ...
                        className={`flex items-center text-xs font-medium px-3 py-1 rounded-full transition-colors duration-200 ${
                          isYoutube
                        ...
                        onClick={
                          isYoutube || isSpotify
                            ? (e) =>
                                handleVideoClick(e, link.url, {
                                  title: song.title,
                                  artist: Array.isArray(song.artists)
                                    ? song.artists.map((a) => a.name).join(", ")
                                    : undefined,
                                  platform: isYoutube ? "youtube" : "spotify",
                                })
                            : undefined
                        }
                      >
                        {link.platform_icon_url ? (
                          <img
                            src={link.platform_icon_url}
                            alt={link.platform_name || "Platform"}
